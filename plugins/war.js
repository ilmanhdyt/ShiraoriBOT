...
*${usedPrefix + command} start* = start game`)
  ...
  if (args[0] == "money"){
    ...
    if(m.sender == conn.war[m.chat][0].users){
      ...
      return conn.reply(m.chat,`*Hanya @${conn.war[m.chat][0].users.split('@')[0]} sebagai pembuat room yang bisa mengganti modal awal perang*`,m, {contextInfo : {mentionedJid : [conn.war[m.chat][0].users]}})
    }
  }

  // JOIN
  if (args[0] == "join"){
    if (global.db.data.userss[m.sender].money < 1000000) return m.reply("*Uang kamu minimal Rp. 1.000.000 untuk bermain game ini.*")
    // FIRST PLAYER
    if (!(m.chat in conn.war)) {
      conn.war2[m.chat] = {"war" : false, "turn" : 0, "time" : 0, "money" : 0}
      conn.war[m.chat] = []
      xp = global.db.data.userss[m.sender].xp
      conn.war[m.chat][0] = {"users": m.sender, "hp": 5000, "lvl": conn.level(global.db.data.userss[m.sender].xp)[0], "turn" : false}
      for (i=1;i<10;i++){
      ...
      for (i = 0 ; i < conn.war[m.chat].length ; i++) {
        if (m.sender == conn.war[m.chat][i].users){
          total = 0
          for (i = 0 ; i < 10 ; i++) {
      ...
      if (args[1]){
        if (args[1].toLowerCase() == "a"){
          if (conn.war2[m.chat].money == 0) return conn.reply(m.chat,`*Tolong @${conn.war[m.chat][0].users.split('@')[0]} tetapkan modal awal perang (minimal Rp. 1.000.000)*\n\n.war money 1.000.000`,m, {contextInfo : {mentionedJid : [conn.war[m.chat][0].users]}})
          if (global.db.data.userss[m.sender].money < conn.war2[m.chat].money) return m.reply(`*Uang kamu minimal Rp. ${conn.war2[m.chat].money.toLocaleString()} untuk bermain game ini.*`)
          for (i = 1 ; i < 5 ; i++) {
            if (conn.war[m.chat][i].users == ""){
              xp = global.db.data.userss[m.sender].xp
              conn.war[m.chat][i] = {"users" : m.sender, "hp" : 5000, "lvl" : conn.level(global.db.data.userss[m.sender].xp)[0], "turn" : false}
              total = 0
              for (i = 0 ; i < 10 ; i++) {
                if (conn.war[m.chat][i].users == ""){
                  total += 1
  ...
  if (args[0] == "start"){
    ...
    for (i=0;i<10;i++){
      if(i<5){
        if (conn.war[m.chat][i].users != "") teamA += 1
      }else{
    ...
    if (teamA == teamB && teamA > 0){
      ...
      for (i=0;i<5;i++){
        if (conn.war[m.chat][i].users != ""){
          users = conn.war[m.chat][i].users
          return conn.reply(m.chat,`*Permainan berhasil dimulai*\n*Silahkan @${users.split('@')[0]} untuk menyerang musuh*\n\n.war player = statistik pemain\n.attack @tag = serang lawan`, m, {contextInfo: { mentionedJid: [users] }})
        }
      }
    }else {
      if (teamA > teamB){
