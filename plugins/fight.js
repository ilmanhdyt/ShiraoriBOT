...
let handler = async (m, { conn, participants }) => {
  ...
  m.reply(`*Kamu* (level ${conn.level(global.db.data.users[m.sender].xp)[0]}) menantang *${conn.getName(lawan)}* (level ${conn.level(global.db.data.users[lawan].xp)[0]}) dan sedang dalam pertarungan sengit.\n\nTunggu ${lamaPertarungan} menit lagi dan lihat siapa yg menang.`)
  ...
  let kesempatan = []
  for (i=0;i<conn.level(global.db.data.users[m.sender].xp)[0];i++) kesempatan.push(m.sender)
  for (i=0;i<conn.level(global.db.data.users[lawan].xp)[0];i++) kesempatan.push(lawan)
  ...
  if (pointPemain > pointLawan){
    ...
    m.reply(`*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${conn.getName(lawan)}*\n\n*Kamu* (level ${conn.level(global.db.data.users[m.sender].xp)[0]}) MENANG melawan *${conn.getName(lawan)}* (level ${conn.level(global.db.data.users[lawan].xp)[0]}) karena kamu ${alasanMenang[getRandom(0,alasanMenang.length-1)]}\n\nHadiah Rp. ${hadiah.toLocaleString()}`)
  }else if (pointPemain < pointLawan){
    let denda = (pointLawan - pointPemain) * 50000
    global.db.data.users[m.sender].money -= denda
    m.reply(`*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${conn.getName(lawan)}*\n\n*Kamu* (level ${conn.level(global.db.data.users[m.sender].xp)[0]}) KALAH melawan *${conn.getName(lawan)}* (level ${conn.level(global.db.data.users[lawan].xp)[0]}) karena kamu ${alasanKalah[getRandom(0,alasanKalah.length-1)]}\n\nUang kamu berkurang Rp. ${denda.toLocaleString()}`)
  }else {
